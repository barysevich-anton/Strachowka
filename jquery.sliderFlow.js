...
(function ($) {
  $.fn.sliderFlow = function (options) {
    ...
    var slide_margin, slide_quantity, slide_width, slide_indent;
    ...
    function initializeSize() {
      if ($_slider.width() < 480) {
        maxSlides = 2;
      } else if ($_slider.width() > 1750) {
        maxSlides = 7;
      } else if ($_slider.width() < 760) {
        maxSlides = 3;
      } else {
        maxSlides = 5;
      }
      ...
      slide_margin =
        $_slider.find(settings.slide).outerWidth(true) -
        $_slider.find(settings.slide).width();
    ...
    function slideTo(e) {
      if (e >= 0 && e <= slide_quantity) {
        $_slider
          .find(settings.slide + ".active-slide")
